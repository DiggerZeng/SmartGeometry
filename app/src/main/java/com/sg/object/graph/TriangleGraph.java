...
//	private boolean isCurveConstrainted = false;

    //cai
    private boolean[] curveConstrainted =  new boolean[2];  //[0]表示三角形内接圆，[1]表示三角形外切圆
...
//		translationStratery = new LineStrategy();  //选择线变换策略
                    ...
                    if(((LineUnit)unit).getType() != 0){
                        ...
                        switch(((LineUnit)unit).getType()){
                            case 1:{			//画图结构temp为垂足。
                                ...
                                this.drawRightAngle(canvas, temp, point1, point2, specialPainter.getPaint(), 20);
                            ...
                            case 3:{		//temp为角平分线约束点
                                PointUnit temp = ((LineUnit)unit).getEndPointUnit();
                                this.drawEqualAngle(canvas, point1, temp, point2, specialPainter.getPaint(), 17);
                                this.drawEqualAngle(canvas, point1, temp, point3, specialPainter.getPaint(), 20);
                                break;
                            }
	...
	*/
    ...
    public boolean isCurveConstrainted() {
        return curveConstrainted[0] || curveConstrainted[1];
    }

    public void setCurveConstrainted(int type, boolean isCurveConstrainted) {
        curveConstrainted[type] = isCurveConstrainted;
    }

    public boolean getCurveConstrainted(int type) {
        return curveConstrainted[type];
    }
